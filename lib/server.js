...
function Socks5Server(options) {
  ...
  this.server = net.createServer(function(connection) {
    ...
    connection.on('request', function(data) {
      ...
      self.onRequest(connection, data, function(err, sock) {
        var buffer = null;
        if (err) {
...
Socks5Server.prototype._udpassociate = function(connection, data, callback) {
  var addrtype = data[3];
  var clientAddress = parseHostPort(data);
  var msgHdrLen = data.length;
  var type = null;
  ...
  if (clientAddress && type) {
    clientAddress.host = connection.remoteAddress;
    ...
    sock.on('message', function(msg, rinfo) {
      ...
      if (rinfo.port === clientAddress.port) {
        ...
        if (address) {
          sock.send(msg, msgHdrLen, msg.length - msgHdrLen, address.port, address.host);
        }
      } else {
        debug('response from remte udp server');
        var buffer = new Buffer(msgHdrLen + msg.length);
        buffer.fill(0);
        buffer[3] = data[3];
 ...
 * +----+-----+-------+------+----------+----------+
 */
Socks5Server.prototype.onRequest = function(connection, data, callback) {
  var command = data[1];
  var addrtype = data[3];
  ...
  switch (command) {
    ...
    case COMMAND.UDP_ASSOCIATE:
      this._udpassociate(connection, data, callback);
      break;
    default:
      return callback({
